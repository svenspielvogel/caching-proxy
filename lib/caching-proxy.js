...
var CachingProxy = function(options) {
    ...
    var SERVER = {
        ...
        checkCache: function (req){
            console.log("caching-proxy.checkCache(url)");
        ...
        onRequestReceived: function (req, res) {
            ...
            //allow head method to check for GET cache
            if(options.method === "HEAD"){
                ...
                if(cache.exists()){
                    cache.sendHeaders(res, 304, {'caching-proxy-available':true});
                }else{
                    cache.sendHeaders(res, 404, {'caching-proxy-available':false});
                }
                res.end();
                return;
            }
